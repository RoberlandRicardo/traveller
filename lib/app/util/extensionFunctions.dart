...
extension MyDateTime on DateTime {
  ...
  String toStringDateBR() {
    String stringDate = "";
    if (day <= 9) {
      stringDate = stringDate + '0';
    }
    stringDate = stringDate + day.toString() + '/';
    if (month <= 9) {
      stringDate = stringDate + '0';
    }
    stringDate = stringDate + month.toString() + '/' + year.toString();

    return stringDate;
  }
}

extension MyDateTimeNull on DateTime? {
  String? toStringBR({required String format}) {
    if (this != null) {
      String stringDate = "";
      if (format == 'date' || format == 'datetime') {
        ...
        stringDate = stringDate + ", " + this!.day.toString() + " de ";
        switch (this!.month) {
          case DateTime.january:
            stringDate = stringDate + "Janeiro";
            break;
        ...
        stringDate = stringDate + " de " + this!.year.toString();
      }
      if (format == 'datetime') {
        stringDate = stringDate + ", as ";
      }
      if (format == 'time' || format == 'datetime') {
        String horaString =
            (this!.hour < 10 ? '0' : '') + this!.hour.toString();
        String minuteString =
